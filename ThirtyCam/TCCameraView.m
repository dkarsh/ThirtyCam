...
@interface TCCameraView () <UIGestureRecognizerDelegate>
...
@interface TCCameraView (InternalMethods) <UIGestureRecognizerDelegate>
...
@interface TCCameraView (CaptureManagerDelegate) <CaptureManagerDelegate>
@end

@implementation TCCameraView
...
- (void) updateDuration
{
    if ([[[self captureManager] recorder] isRecording])
    {
        ...
        self.durationProgressBar.progressTintColor = [UIColor blueColor];
...
- (void) saveVideoWithCompletionBlock:(void(^)(NSURL  *urlSuccess))completion {
    ...
    [self.captureManager saveVideoWithCompletionBlock:^(NSURL *urlSuccess) {
        ...
        if (urlSuccess)
        {
            ...
            completion? completion(urlSuccess):(nil);
        }
        else
        {
...
@implementation TCCameraView (InternalMethods)
...
@implementation TCCameraView (CaptureManagerDelegate)

- (void) updateProgress
{
    self.durationProgressBar.progressTintColor = [UIColor greenColor];
    self.progressView.hidden = NO;
    self.durationProgressBar.hidden = NO;
    self.activityView.hidden = YES;
    self.progressLabel.text = @"Creating the video";
    self.durationProgressBar.progress = self.captureManager.exportSession.progress;
    if (self.durationProgressBar.progress > .99) {
